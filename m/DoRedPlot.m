...
plot(tout,torque,'r','LineWidth',2);
title('Engine');
ylabel('Torque [Nm]');
%axis([StartTime inf -inf inf]);
...
plot(tout,3.6*v,'magenta','LineWidth',2);
title('Vehicle');
...
u = 1-1./(1+uSAE);
plot(u,Fx,'r','LineWidth',1); plot(kappa(1),interp1(u,Fx,kappa(1)),'r.');
h = legend('Characteristic','Operation points');
set(h,'Location','SouthEast','FontSize',6);
plot(kappa,interp1(u,Fx,kappa),'r.');
...
% Drive shaft torsion
subplot(326); hold on; grid on;
plot(tout,thDiff_d,'r','LineWidth',2);
title('Drive shafts');
xlabel('Time [s]');
ylabel('Torsion [rad]');
%axis([StartTime inf -inf inf]);
